...
def multi_head_attention(query: torch.Tensor, key: torch.Tensor, value: torch.Tensor, attention_mask: torch.Tensor = None):
    ...
    if attention_mask is not None:
        # attention_mask is (batch_size, total_len, total_len) with 0 for "can attend" and large negative for "cannot attend"
        ...
        if attention_mask is not None:
            ...
            attention_mat = attention_mat + attention_mask  # Add mask directly (0 for valid, large negative for invalid)
